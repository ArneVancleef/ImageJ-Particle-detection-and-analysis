//Go to imageJ → Plugin → New → Plugin : Paste this code and press run to start the macro. 
...
//In the "manage update sites" select BioVoxxel, ImageScience, Morphology, then close manage update site and then press apply changes. After updating restart imageJ. 
...
var inNumberClosingCycles = 0;								//Closing cycles (dilation + erosion) after putting together the intensity thresholding
var FOVcfYesOrNo = true;									//Boolean: use a field of view correction factor in the analysis
var AddScaleBar = true;										//Boolean: add scale bars to the original images after analysis
...
var ErosionCycles = 3;										//Erosion cycles for watershed irregular features: see biovoxxel toolbox wiki
...
var minSize = 1; 											//Minmum Size of a particle to be analyzed
var maxSize = 400;
...
Dialog.addMessage("");
Dialog.addNumber("scale in pixels/µm", inNumberScale);
Dialog.addChoice("file type:", newArray("Preview (press ok for preview)", "Map of videos (avi.)", "Single image", "Map with maps of images")); 
...
Dialog.addNumber("Edge detection High Threshold", inNumberEdgeHighThreshold);
Dialog.addNumber("Edge detection Low Threshold", inNumberEdgeLowThreshold);
Dialog.addNumber("Edge detection Smoothing", inNumberEdgeComputeSmoothing);
Dialog.addSlider("Edge detection Sharpening", 0, 0.9, inNumberEdgeUnsharpMask);
Dialog.addNumber("Frames for background (videos/maps)", FramesForBackground);
...
Dialog.addSlider("Gaussian Blur radius (intensity threshold)", 0, 10, GaussianBlurBeforeIntensityThresholding);  
Dialog.addSlider("Closing cycles after intensity thresholding", 0, 10, inNumberClosingCycles);
Dialog.addCheckbox("Field of View correction factor", FOVcfYesOrNo); 
Dialog.addCheckbox("Add scalebar to input", AddScaleBar); //adds scalebar to input images after analysis
Dialog.addCheckbox("Show processing steps (single image)", ShowProcessingStepsBoolean);
Dialog.addSlider("minimum particle size in pixels", 1, 1000, minSize);
Dialog.addSlider("maximum particle size in pixels", 1, 10000, maxSize);
Dialog.addNumber("minimum particle circularity (0-1)", minCircularity);
Dialog.addNumber("maximum particle circularity (0-1)", maxCircularity);
...
Dialog.addNumber("watershed IF erosion cycles", ErosionCycles);
Dialog.addNumber("watershed IF convexity threshold", WatershedConvexity);
Dialog.addCheckbox("Extended results", ExtendedResults);
Dialog.show();

// Once the Dialog is OKed the rest of the code is executed
// values are recovered in order of appearance
inNumberScale = Dialog.getNumber(); //Check
ChoiceType  = Dialog.getChoice(); //Check 
ChoiceBackground  = Dialog.getChoice(); //Check
inNumberEdgeHighThreshold = Dialog.getNumber(); //Check
inNumberEdgeLowThreshold = Dialog.getNumber(); //Check
inNumberEdgeComputeSmoothing = Dialog.getNumber(); //Check
inNumberEdgeUnsharpMask = Dialog.getNumber(); //Check
FramesForBackground = Dialog.getNumber();   //check
...
GaussianBlurBeforeIntensityThresholding = Dialog.getNumber();
inNumberClosingCycles = Dialog.getNumber(); //Check
FOVcfYesOrNo = Dialog.getCheckbox(); // Check
AddScaleBar = Dialog.getCheckbox();
ShowProcessingStepsBoolean = Dialog.getCheckbox();
minSize = 2*3.141*Math.sqr(Dialog.getNumber())/4;
maxSize = 2*3.141*Math.sqr(Dialog.getNumber())/4;
...
if(WatershedIrregularFeaturesBoolean == true)
{
ErosionCycles = Dialog.getNumber();
WatershedConvexity = Dialog.getNumber();
} 
ExtendedResults = Dialog.getCheckbox();
	...
	run("Duplicate...", "title=AfterCannyEdgeDetectionAndMorphologicalOperators");  
		...
		run("EDM Binary Operations", "iterations=2 operation=dilate");
			...
			if(HistogramBinerization == true)
			{
				...
				if(OnlyHistogramBinerization == false){
			...
			if(WatershedIrregularFeaturesBoolean == true){
				run("Watershed Irregular Features", "erosion="+ErosionCycles+" convexity_threshold="+WatershedConvexity+" separator_size=0-Infinity");
			} else if (WatershedBoolean == true){
				run("Watershed");
			}
...
OrigineleAfbeelding = getTitle(); 
	run("Duplicate...", "title=BackgroundSubstracted");
	run("8-bit"); 
			if(ChoiceBackground == "Rolling ball from single image")
			{
				makeRectangle(0, 0, getWidth(), getHeight());
...
//------------------------------			
			...
			//Getting necessary information out of results
				...
				{
					Table.applyMacro("Diameter=sqrt(Area/3.14*4); Diameter3=Diameter*Diameter*Diameter; Diameter4=Diameter3*Diameter; Diameter2=Diameter*Diameter");
					...
					diameter2Measurements = newArray(nResults);
					for(k=0; k<nResults; k++) 
					{
					    ...
					    diameter2Measurements[k] = getResult("Diameter2", k);  
					}
				}
				else if (FOVcfYesOrNo == true)
				{
					Table.applyMacro("Diameter=sqrt(Area/3.14*4); Diameter3=Diameter*Diameter*Diameter; Diameter4=Diameter3*Diameter; FOVcf=("+FinalWidthImage+"*"+FinalHeightImage+")/(("+FinalHeightImage+"-Height)*("+FinalWidthImage+"-Width)); Diameter2=Diameter*Diameter");
					FOVcfMeasurements = newArray(nResults);
					diameterMeasurements = newArray(nResults);
					diameter3Measurements = newArray(nResults);
					diameter4Measurements = newArray(nResults);
						...
						for(h=totalCount; h<totalCount+getResult("Count",m);h++)
    						{
								...
								D2som = D2som + (diameter2Measurements[h]*FOVcfMeasurements[h]);
			...
			run("Close");
			print(diameterMeasurements[h-1]);
			print(FOVcfMeasurements[h-1]*4*3.141*(diameterMeasurements[h-1]*diameterMeasurements[h-1]*diameterMeasurements[h-1])/(8*3));
			...
			print("erosion cycles: "+ ErosionCycles);
...
if (ChoiceType == "Map with maps of images" )
{
setBatchMode(true);
dir = getDirectory("Choose a folder");
listTwo = getFileList(dir);
