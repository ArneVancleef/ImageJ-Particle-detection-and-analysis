...
var inNumberScale = 0.7265;									//scale in pixel/µm: must be calculated from calibration image then go to ImageJ toolbar → Analyze → Set scale...
...
var inNumberEdgeHighThreshold = 7;							//High threshold value for the edge detection, typically between 2 or 15, depends on image/compute smoothing and unsharp mask, higher thresholds mean lower sensitivity
    ...
    for (i = 0; i < listTwo.length; i++) 
    {
        ...
        if(File.isDirectory(dir + File.separator + listTwo[i]) && matches(mapname,".*Input*."))
        {
			...
			for (n = 0; n < listThree.length; n++) 
				{  		
					if(ChoiceBackground == "Median of multiple frames (videos/maps)"){
					open(dir + videonaam2 +"Outputbackground.tif");
					rename("background.tif"); 
					}
